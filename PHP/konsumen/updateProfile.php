...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['gambar']['name'])) {
        ...
        if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $targetDir.$fileMenu)) {
            ...
            $query = "UPDATE tb_user t1 JOIN tb_konsumen t2 ON (t1.id_user = t2.id_konsumen) SET t1.username='$username', t1.password='$password', t1.gambar='$gambar', t2.name = '$nama', t2.nohp = '$nohp' WHERE t1.id_user='$id_user'";
        ...
        $query = "UPDATE tb_user t1 JOIN tb_konsumen t2 ON (t1.id_user = t2.id_konsumen) SET t1.username='$username', t1.password='$password', t2.name = '$nama', t2.nohp = '$nohp' WHERE t1.id_user='$id_user'";
        $execute = mysqli_query($conn, $query);
        $check = mysqli_affected_rows($conn);
