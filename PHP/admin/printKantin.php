    ...
    <div class="container">
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php 
                        $querykeluhan = $conn->prepare("SELECT tb_user.username, tb_user.role, tb_penjual.name AS name, tb_penjual.nohp AS nohp FROM tb_user LEFT JOIN tb_penjual ON tb_user.id_user = tb_penjual.id_penjual WHERE tb_user.role='kantin'");
                ...
                <tfoot>
                    <?php
                        ...
                        $querytKantin = $conn->prepare("SELECT COUNT(*) AS JML FROM `tb_user` WHERE role='kantin'");
                        $querytKantin->execute();
                        $resultKantin = $querytKantin->get_result();
                        $rowKantin = $resultKantin->fetch_assoc();
                        $Kantin = $rowKantin['JML'];
                    ?>
                    <td colspan="10">Total Kantin : <?php echo ($Kantin); ?><br>
                                    </td>
                </tfoot>
            </table>
        </div>
